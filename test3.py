...
def bureau_features() -> pd.DataFrame:
    ...
    recent_24m = aggregate_table(bureau[bureau["DAYS_CREDIT"] >= -730], "SK_ID_CURR", "BURO_RECENT_24M")
    ...
    for recent in (recent_12m, recent_24m):
        if not recent.empty:
            full_agg = full_agg.merge(recent, on="SK_ID_CURR", how="left")
...
def previous_application_features() -> pd.DataFrame:
    ...
    recent_12m = aggregate_table(prev[prev["DAYS_DECISION"] >= -365], "SK_ID_CURR", "PREV_RECENT_12M")
    recent_24m = aggregate_table(prev[prev["DAYS_DECISION"] >= -730], "SK_ID_CURR", "PREV_RECENT_24M")
...
def pos_cash_features() -> pd.DataFrame:
    ...
    recent_12m = aggregate_table(pos[pos["MONTHS_BALANCE"] >= -12], "SK_ID_CURR", "POS_RECENT_12M")
...
def installments_features() -> pd.DataFrame:
    ...
    recent_12m = aggregate_table(ins[ins["DAYS_INSTALMENT"] >= -365], "SK_ID_CURR", "INSTAL_RECENT_12M")
...
def credit_card_features() -> pd.DataFrame:
    ...
    recent_12m = aggregate_table(cc[cc["MONTHS_BALANCE"] >= -12], "SK_ID_CURR", "CC_RECENT_12M")
...
def train_and_predict(train_df: pd.DataFrame, test_df: pd.DataFrame) -> tuple[np.ndarray, float, dict[str, float]]:
    ...
    for fold, (train_idx, valid_idx) in enumerate(skf.split(X, y), start=1):
        ...
        model_oof["lgb"][valid_idx] = lgb_oof
        model_oof["xgb"][valid_idx] = xgb_oof
        ...
        if USE_CATBOOST:
            ...
            cat_model.fit(X_train, y_train, eval_set=(X_valid, y_valid), early_stopping_rounds=200)
    ...
    return final_test, full_auc, best_weights
...
def main() -> None:
    ...
    preds, auc, weights = train_and_predict(train_df, test_df)
    ...
    output_path = Path(__file__).resolve().parent / "submission3.csv"
