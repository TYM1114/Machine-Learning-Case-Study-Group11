...
MAX_FEATURES = 900
EXT_SOURCE_TARGETS = ("EXT_SOURCE_1", "EXT_SOURCE_2", "EXT_SOURCE_3")
...
def add_ext_source_prediction_features(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    predictor_df = df.drop(columns=["TARGET"], errors="ignore").copy()

    usable_cols: list[str] = []
    for col in predictor_df.columns:
        if col in {"SK_ID_CURR", *EXT_SOURCE_TARGETS}:
            continue
        if predictor_df[col].isna().all():
            continue

        if pd.api.types.is_numeric_dtype(predictor_df[col]):
            predictor_df[col] = predictor_df[col].fillna(-999)
            usable_cols.append(col)
        else:
            codes, _ = pd.factorize(predictor_df[col].fillna("__MISSING__"))
            predictor_df[col] = codes.astype(np.int32)
            usable_cols.append(col)

    predictors = predictor_df[usable_cols]

    for target_col in EXT_SOURCE_TARGETS:
        available_mask = df[target_col].notnull()
        if available_mask.sum() < 5000:
            continue
        ...
        model.fit(predictors.loc[available_mask], df.loc[available_mask, target_col])

        pred_col = f"APP_{target_col}_PRED"
        diff_col = f"APP_{target_col}_DIFF"
        missing_col = f"APP_{target_col}_MISSING"

        df[pred_col] = model.predict(predictors)
        df[diff_col] = df[target_col] - df[pred_col]
        df[missing_col] = df[target_col].isnull().astype(np.int8)
        df[diff_col] = df[diff_col].fillna(0)

    pred_cols = [f"APP_{col}_PRED" for col in EXT_SOURCE_TARGETS if f"APP_{col}_PRED" in df.columns]
    diff_cols = [f"APP_{col}_DIFF" for col in EXT_SOURCE_TARGETS if f"APP_{col}_DIFF" in df.columns]
    if pred_cols:
        df["APP_EXT_SOURCE_PRED_MEAN"] = df[pred_cols].mean(axis=1)
        df["APP_EXT_SOURCE_PRED_STD"] = df[pred_cols].std(axis=1)
    if diff_cols:
        df["APP_EXT_SOURCE_DIFF_ABS_SUM"] = df[diff_cols].abs().sum(axis=1)
        df["APP_EXT_SOURCE_DIFF_STD"] = df[diff_cols].std(axis=1)
...
def build_dataset() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    print("Building EXT_SOURCE prediction features...")
    app = add_ext_source_prediction_features(app)
...
def main() -> None:
    ...
    output_path = Path(__file__).resolve().parent / "submission5.csv"
